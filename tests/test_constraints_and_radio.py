...
# The two known radio groups this repo maps as "fill": "manual".
EXPECTED_MANUAL = {
    "MRS-1041ME": ("do_you_want_to_allow_someone_to_discuss_return_with_mrs",
                   ["Yes", "No"]),
    "MRS-700SOV": ("residency_status", ["Resident", "Nonresident"]),
}
...
class ManualRadioMappings(unittest.TestCase):
    def test_known_radio_groups_are_declared_manual(self):
        for fid, (group_fid, options) in EXPECTED_MANUAL.items():
            mapping = json.loads((FORMS / fid / "mapping.json").read_text())
            spec = (mapping.get("manual") or {}).get(group_fid)
            self.assertIsNotNone(spec, f"{fid}: {group_fid} not in manual")
    ...
    def test_sample_case_resolves_a_suggestion(self):
        ...
        for fid, (group_fid, _) in EXPECTED_MANUAL.items():
            case = json.loads(
            ...
            e = entries[group_fid]
            self.assertEqual(e["action"], "manual selection required")
    ...
    def test_fill_reports_untouched_radio_with_suggestion(self):
        ...
        for fid, (group_fid, _) in EXPECTED_MANUAL.items():
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                # post-fill expectation: the radio group is untouched —
                # every button still off, none deleted, NOT an error
